...
/*		  System.out.println("fullHttpRequest.content :" + fullHttpRequest.content().readableBytes());
		  byte[] inputBytes = new byte[fullHttpRequest.content().readableBytes()];
		  for(int i=0;i<fullHttpRequest.content().readableBytes();i++){
			  inputBytes[i] = fullHttpRequest.content().readByte();
		  }
		  System.out.println("hexStr content = " + StringUtil.parseByte2HexStr(inputBytes));
*/		  
		  
//		  ByteBufInputStream inputStream  = new ByteBufInputStream(fullHttpRequest.content());
		  
		  if(fullHttpRequest.content().hasArray()){
			  byte[] connectArray = fullHttpRequest.content().array();
			  log.info("content has array: " + new String(connectArray,"UTF-8").equals(content));
			  log.info("content has array: " + new String(connectArray,"UTF-8"));
		  }
		  httpParameter.setContent(content);
		  
		  readingChunks = HttpHeaders.isTransferEncodingChunked(fullHttpRequest);
//		  log.debug("Is Chunked: " + readingChunks + "\r\n");
//		  log.debug("IsMultipart: " + decoder.isMultipart() + "\r\n");
		  try {
